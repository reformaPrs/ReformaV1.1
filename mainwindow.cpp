...
{
    if(ui->tabsWidget->getProgramWidget()->isVisible() || (programWidgetViewPushButton->isVisible() && ui->graphicsView->isVisible())){
        saveToMioconfig();
    }
    else if(ui->graphicsView->isVisible() && intervalPushButton->isVisible()){
        saveToMio();
    }
    ...
    QString fileName = QFileDialog::getOpenFileName(this,
                                                    tr("Open file"), "",
                                                    tr("Reforma (*.mioconfig);;(*.mio);;All Files (*)"));
...
QString MainWindow::cutString(const QString &string)
{
...
void MainWindow::saveToMioconfig()
{
    updateProgram();

    QMessageBox msgBox;
    msgBox.setWindowIcon(QIcon(":/icons/icons/reforma1.ico"));
    msgBox.setText(tr("Program will be closed. Do you want save it in export file?"));
    msgBox.setIcon(QMessageBox::Question);
    QPushButton *yesBtn = msgBox.addButton(tr("Yes"), QMessageBox::ActionRole);
    QPushButton *noBtn = msgBox.addButton(tr("No"), QMessageBox::ActionRole);
    msgBox.exec();
    ...
    if (msgBox.clickedButton() == (QAbstractButton *) yesBtn){
        ...
        if(fileName.isEmpty())
            ...
            *out << fileName + " " + cutString(ui->tabsWidget->getCurrentProgramName());

            ProgramWidget::createConfigData(out);

            ui->tabsWidget->getProgramWidget()->setProgramValuesForSave();
            updateProgram();
